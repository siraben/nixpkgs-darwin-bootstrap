  ...
  phase36-gcc46-libgcc =
    if hostPlatform.isx86_64 then
      runCommand "darwin-minimal-bootstrap-phase36-gcc-${gcc46Version}-libgcc-amd64" { } ''
        mkdir -p work $out/lib/gcc/x86_64-apple-darwin/${gcc46Version} $out/share/darwin-bootstrap
        ...
        root = os.getcwd().encode()
        pattern = re.compile(rb"/nix/var/nix/builds/[^/\0]+/(src|build)")

        def repl(match):
            return root + b"/work/" + match.group(1)

        for base in ("work/src", "work/build"):
            for directory, dirs, files in os.walk(base):
                ...
                for name in files:
                    ...
                    with open(path, "rb") as handle:
                        data = handle.read()
                    replaced = pattern.sub(repl, data)
                    if replaced != data:
                        with open(path, "wb") as handle:
                            handle.write(replaced)
        PY

        for suffix in h def md opt c; do
          find work/build/gcc -maxdepth 1 -type f -name "*.$suffix" -exec cp -n {} work/src/gcc/ \;
        done
        find work/src/include -maxdepth 1 -type f -name '*.h' -exec cp -n {} work/src/gcc/ \;
        find work/src/gcc -maxdepth 1 -type f \
          \( -name '*.h' -o -name '*.def' -o -name '*.md' -o -name '*.opt' \) \
          -exec cp -n {} work/src/gcc/config/ \;

        ln -snf .. work/src/gcc/config/i386/config
        cp work/build/gcc/sfp-machine.h work/src/gcc/config/soft-fp/sfp-machine.h
        ln -snf ../../../libgcc/config work/src/gcc/config/soft-fp/config
        rm -f work/src/gcc/config/soft-fp/longlong.h
        ln -snf ../../longlong.h work/src/gcc/config/soft-fp/longlong.h
        cp work/build/gcc/include/unwind.h work/src/gcc/unwind.h
        printf '#include "gthr-single.h"\n' > work/build/gcc/gthr-default.h
        printf '#include "gthr-single.h"\n' > work/src/gcc/gthr-default.h
        cp work/build/gcc/include/stdarg.h work/src/libgcc/stdarg.h
        cat ${phase34-tinycc-darwin-cc}/include/tcc-darwin-bootstrap/fcntl.h > work/src/libgcc/fcntl.h
        cat >> work/src/libgcc/fcntl.h <<'H'
        #ifndef F_RDLCK
        #define F_RDLCK 1
        #define F_WRLCK 3
        #define F_SETLKW 9
        ...
        cat > work/build/gcc/as <<'SH'
        #! /bin/sh
        set -eu
        compiler="${phase34-tinycc-darwin-cc}/bin/tcc-darwin-cc"
        out=""
        input=""
        while test "$#" -gt 0; do
          case "$1" in
        ...
        gcov_io = Path("work/src/gcc/gcov-io.c")
        ...
        mvars = Path("work/build/gcc/libgcc.mvars")
        lines = mvars.read_text().splitlines()
        out = []
        for line in lines:
        ...
        libgccDir=work/build/x86_64-apple-darwin/libgcc
        mkdir -p work/build/gcc/include work/build/gcc/include/sys
        find ${phase34-tinycc-darwin-cc}/include/tcc-darwin-bootstrap -maxdepth 1 -type f -name '*.h' -exec cp -n {} work/build/gcc/include/ \;
        find ${phase34-tinycc-darwin-cc}/include/tcc-darwin-bootstrap/sys -maxdepth 1 -type f -name '*.h' -exec cp -n {} work/build/gcc/include/sys/ \;
        mkdir -p "$libgccDir"
        mkdir -p "$libgccDir/include"
        cp work/src/libgcc/stdarg.h "$libgccDir/include/stdarg.h"
        cp work/src/libgcc/fcntl.h "$libgccDir/include/fcntl.h"

        ${python3}/bin/python3 - <<'PY'
        from pathlib import Path
        makefile = Path("work/src/libgcc/Makefile.in")
        ...
        find "$libgccDir" \( -name '*.o' -o -name '*.a' -o -name '*.dep' \) -print0 | xargs -0 rm -f

        cd "$libgccDir"
        export MACOSX_DEPLOYMENT_TARGET=10.6
        targetCc="$PWD/../../gcc/xgcc -B$PWD/../../gcc/ -B${phase35-gcc46-all-gcc}/x86_64-apple-darwin/bin/ -B${phase35-gcc46-all-gcc}/x86_64-apple-darwin/lib/ -isystem ${phase35-gcc46-all-gcc}/x86_64-apple-darwin/include -isystem ${phase35-gcc46-all-gcc}/x86_64-apple-darwin/sys-include --sysroot=${phase34-tinycc-darwin-cc}/include/tcc-darwin-bootstrap"
        CC="$targetCc" \
        CPP="$targetCc -E" \
        AR="$AR" \
        RANLIB="$RANLIB" \
