 ...
 *
 * Exception: the bake chain links gcc-compiled objects (e.g. gcc-10's
 * build/genmatch) with the chain tcc link path, NOT binutils ld — they
 * resolve against the bootstrap libc, whose plain `fstat`/`stat` already do
 * the 64-bit stat64 syscall and fill this 64-bit `struct stat`.  There the
 * `$INODE64` symbols don't exist (the chain link reports e.g. "Target label
 * ELF_fstat_INODE64 is not valid"), so bake defines __BAKE_PLAIN_STAT__ to
 * keep the plain symbol for gcc too.
 */
#if defined(__GNUC__) && !defined(__TINYC__) && !defined(__BAKE_PLAIN_STAT__)
