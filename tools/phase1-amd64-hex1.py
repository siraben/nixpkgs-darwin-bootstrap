...
def macho_header():
    ...
    commands.append(p32(0x1B) + p32(24) + bytes.fromhex("102132435465768798a9bacbdcedfe0f"))
    commands.append(
        p32(0x32)
        + p32(32)
        + p32(1)
    ...
    return header + bytes(entry - len(header)), base, data_vm, linkedit_offset


def port_hex1_source(stage0_sources, output):
    source = (stage0_sources / "AMD64/hex1_AMD64.hex0").read_text().split("#:ELF_text", 1)[1]
...
def patch_binary(binary, data_vm):
    for opcode in [
        ...
        patch_rel32(binary, opcode, data_vm)

    pattern = bytes.fromhex("31c00f054885c0740b")
    patch_position = binary.find(pattern)
    if patch_position < 0:
        raise ValueError("read syscall pattern not found")

    stub_position = max(len(binary) + 0x800, patch_position + 0x100)
    if len(binary) < stub_position:
        binary.extend(bytes(stub_position - len(binary)))

    binary[patch_position : patch_position + 7] = (
        b"\xE9" + struct.pack("<i", stub_position - (patch_position + 5)) + b"\x90\x90"
    )
    stub = (
        bytes.fromhex("b8030000020f054885c0")
        + b"\xE9"
        + struct.pack("<i", (patch_position + 7) - (stub_position + 15))
    )
    binary[stub_position : stub_position + len(stub)] = stub


def main():
    stage0_sources = pathlib.Path(sys.argv[1])
    hex0 = pathlib.Path(sys.argv[2])
    output_dir = pathlib.Path(sys.argv[3])
    output_dir.mkdir(parents=True, exist_ok=True)

    body_source = output_dir / "hex1_AMD64_darwin_body.hex0"
    port_hex1_source(stage0_sources, body_source)
    body_binary = output_dir / "hex1-body.bin"
    subprocess.check_call([str(hex0), str(body_source), str(body_binary)])
