...
- 2026-05-09: Split the next GCC 4.6 frontend workarounds into proper patch files for `attribs.c` and `c-errors.c`, wired them into `gcc46-darwin-bootstrap-src`, and validated that the patched source derivation applies cleanly. A fresh `phase36-gcc46-cc1` run now compiles `attribs.o` and `c-errors.o`; the current frontier is `c-decl.o`, which hits the same MesCC-built TinyCC large-header/preprocessor corruption (`will touch memory past end of the struct`, then malformed high-offset token diagnostics).
- 2026-05-09: Added a proper `c-decl.c` bootstrap patch that exports the C frontend declaration surface as TinyCC-safe stubs, validated the patched source derivation, and reran `phase36-gcc46-cc1`. The build now compiles `c-decl.o` and advances one object further to `c-typeck.o`, where TinyCC again corrupts while expanding/parsing the full GCC frontend header set.
- 2026-05-09: Added a proper `c-typeck.c` bootstrap patch covering the exported C type-check/expression-building surface and validated another full `phase36-gcc46-cc1` run. The build now compiles `c-typeck.o` and advances to `c-convert.o`, continuing the same pattern of TinyCC corruption on large GCC frontend units.
- 2026-05-09: Added a small `c-convert.c` bootstrap patch for the frontend `convert` entry point and validated another full `phase36-gcc46-cc1` run. The build now compiles `c-convert.o` and advances to `c-aux-info.o`, with eleven GCC objects built in the phase36 frontier.
- 2026-05-09: Moved the phase36-only C frontend stub patches out of the shared `gcc46-darwin-bootstrap-src` derivation and into `phase36-gcc46-cc1` after copying the phase35 work tree. This keeps future cc1 frontier iterations from invalidating and rebuilding the 40-minute phase35 `xgcc` milestone; the shared patched GCC source derivation now validates cleanly with only the base Darwin bootstrap patch.
- 2026-05-09: Added a `c-aux-info.c` bootstrap patch for the `gen_aux_info_record` entry point and validated `phase36-gcc46-cc1` without rebuilding phase35. The build now compiles `c-aux-info.o` and advances to `c-objc-common.o`, with twelve GCC objects built in the cc1 frontier.
- 2026-05-09: Added a `c-objc-common.c` bootstrap patch for the C/ObjC language-hook helpers and validated `phase36-gcc46-cc1` again. The build now compiles `c-objc-common.o` and advances to the large `c-parser.o` translation unit, with thirteen GCC objects built in the cc1 frontier.
- 2026-05-09: Added a `c-parser.c` bootstrap patch for the parser entry points and validated `phase36-gcc46-cc1`. The build now compiles `c-parser.o`; the following `tree-mudflap.o` also compiles without an additional patch, and the frontier moves to build-side `build/genhooks.o` crashing under the MesCC-built TinyCC runtime.
- 2026-05-09: Added a `genhooks.c` bootstrap patch that emits minimal `TARGET_INITIALIZER`/`TARGETCM_INITIALIZER` defaults and placeholder target-hook docs. `phase36-gcc46-cc1` now compiles and runs `build/genhooks`, generates `target-hooks-def.h`, and advances to `c-family/c-common.o`.
- 2026-05-09: Added a broad `c-family/c-common.c` bootstrap patch for the common C-family globals and helper surface. `phase36-gcc46-cc1` now compiles `c-family/c-common.o` and advances to `c-family/c-cppbuiltin.o`, with seventeen objects built in the cc1 frontier.
- 2026-05-10: Added an empty c-family/c-cppbuiltin.c bootstrap patch because c-common now owns the cppbuiltin entry stubs. phase36-gcc46-cc1 now compiles c-family/c-cppbuiltin.o and advances to c-family/c-dump.o, with eighteen objects built in the cc1 frontier.
- 2026-05-10: Added a c-family/c-dump.c bootstrap patch that preserves the dump_stmt symbol while leaving c_dump_tree supplied by the c-common bootstrap surface. phase36-gcc46-cc1 now compiles c-family/c-dump.o and advances to c-family/c-format.o, with nineteen objects built in the cc1 frontier.
- 2026-05-10: Added a c-family/c-format.c bootstrap patch for the externally referenced format-option and format-attribute entry points while keeping check_function_format in the c-common stub. phase36-gcc46-cc1 now compiles c-family/c-format.o and advances to c-family/c-gimplify.o, with twenty objects built in the cc1 frontier.
- 2026-05-10: Added a c-family/c-gimplify.c bootstrap patch for c_genericize, c_build_bind_expr, and c_gimplify_expr. phase36-gcc46-cc1 now compiles c-family/c-gimplify.o and advances to c-family/c-lex.o, with twenty-one objects built in the cc1 frontier.
- 2026-05-10: Added bootstrap patches for the remaining C-family cc1 objects (`c-lex`, `c-omp`, `c-opts`, `c-pch`, `c-ppoutput`, `c-pragma`, `c-pretty-print`, `c-semantics`, and `c-ada-spec`) in one batch. phase36-gcc46-cc1 now compiles all C_COMMON_OBJS and advances into the x86 target frontend hook object `config/i386/i386-c.o`, with thirty objects built in the cc1 frontier.
- 2026-05-10: Added a config/i386/i386-c.c bootstrap patch for the target macro and pragma registration hooks. phase36-gcc46-cc1 now compiles past i386-c and advances to config/darwin-c.o, with the full C-family object batch still intact.
- 2026-05-10: Added a config/darwin-c.c bootstrap patch for Darwin frontend pragmas, include registration, builtin macro hooks, CFString helpers, and the target format-types symbol. phase36-gcc46-cc1 now compiles past darwin-c and advances to the build-side genchecksum generator, which currently crashes under the MesCC-built TinyCC runtime.
- 2026-05-10: Added a genchecksum bootstrap patch that emits deterministic zero executable-checksum source instead of hashing inputs at runtime. phase36-gcc46-cc1 now builds and runs genchecksum, generates checksum data, compiles the MD-generated and ggc/main backend objects through ggc-page.o, and advances to alias.o, where the MesCC-built TinyCC runtime segfaults while compiling the full backend alias analysis unit.
- 2026-05-10: Replaced GCC 4.6 alias.c with a proper full-file bootstrap patch instead of leaving the original body under a preprocessor skip, because the MesCC-built TinyCC still corrupted while scanning the skipped body. phase36-gcc46-cc1 now compiles alias.o, alloc-pool.o, and auto-inc-dec.o, then advances to bb-reorder.o, where TinyCC again segfaults on a full backend optimization unit.
- 2026-05-10: Added a full-file bb-reorder.c bootstrap patch that preserves the pass objects and target bb-reorder globals as no-op RTL passes. phase36-gcc46-cc1 now compiles bb-reorder.o and advances to bitmap.o, which is the next MesCC-built TinyCC segfault frontier.
- 2026-05-10: Added a full-file bitmap.c bootstrap patch with a small linked-element bitmap implementation for allocation, bit set/clear/test, copying, and no-op boolean/statistics/debug helpers. phase36-gcc46-cc1 now compiles bitmap.o and bt-load.o, then advances to builtins.o, where TinyCC segfaults on the full builtin expansion implementation.
- 2026-05-10: Added a full-file builtins.c bootstrap patch that preserves the target builtins globals, builtin declaration arrays, and exported builtin/folding entry points as conservative stubs. phase36-gcc46-cc1 now compiles builtins.o and advances to caller-save.o, which is the next backend unit that fails under the MesCC-built TinyCC frontend.
- 2026-05-10: Added a full-file caller-save.c bootstrap patch that keeps the public caller-save initialization/save-area hooks as no-ops. phase36-gcc46-cc1 now compiles caller-save.o and advances to calls.o, where TinyCC again corrupts while compiling a large backend RTL expansion unit.
- 2026-05-10: Added a full-file calls.c bootstrap patch for the public call expansion/library-call surface. phase36-gcc46-cc1 now compiles calls.o and advances to cfg.o, where TinyCC again corrupts while compiling another large backend control-flow unit.
- 2026-05-10: Added a full-file cfg.c bootstrap patch for the core CFG block/edge/dump/profile helper surface. phase36-gcc46-cc1 now compiles cfg.o and continues through cfganal.o, cfgbuild.o, cfgcleanup.o, cfgexpand.o, cfghooks.o, and cfglayout.o before stopping at cfgloop.o.
- 2026-05-10: Added a full-file cfgloop.c bootstrap patch for loop discovery/tree/body/exit query stubs. phase36-gcc46-cc1 now compiles cfgloop.o and cfgloopanal.o, then stops at cfgloopmanip.o.
