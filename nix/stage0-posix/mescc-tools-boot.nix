...
# Warning all binaries prior to the use of blood-macho will not be readable by
...
rec {
  ...
  M0_hex2 = run "M0.hex2" catm [
    out
    "${m2libc}/${m2libcArch}/${executableHeader}"
  ...
  ############################################
  # Phase-6 Build blood-macho-0 from C sources #
  ############################################

  blood-macho-0_M1 = run "blood-macho-0.M1" M2 [
    "--architecture"
    m2libcArch
    "-f"
    "${m2libc}/${m2libcArch}/${bootstrapC}"
  ...
  blood-macho-0-0_M1 = run "blood-macho-0-0.M1" catm [
    out
    "${m2libc}/${m2libcArch}/${m2libcArch}_defs.M1"
    "${m2libc}/${m2libcArch}/${libcCore}"
    blood-macho-0_M1
  ];
  blood-macho-0_hex2 = run "blood-macho-0.hex2" M0 [
    blood-macho-0-0_M1
    out
  ];
  blood-macho-0-0_hex2 = run "blood-macho-0-0.hex2" catm [
    out
    "${m2libc}/${m2libcArch}/${executableHeader}"
    blood-macho-0_hex2
  ];
  blood-macho-0 = run "blood-macho-0" hex2-0 [
    blood-macho-0-0_hex2
    out
  ];
  ...
  M1-macro-0-footer_M1 = run "M1-macro-0-footer.M1" blood-macho-0 (
    bloodFlags
    ++ [
      "-f"
      M1-macro-0_M1
  ...
  hex2_linker-0_M1 = run "hex2_linker-0.M1" M2 [
    ...
    "-f"
    "${m2libc}/${m2libcArch}/${m2libcOS}/unistd.c"
    "-f"
    "${m2libc}/${m2libcArch}/${m2libcOS}/fcntl.c"
    "-f"
    "${m2libc}/fcntl.c"
    "-f"
    "${m2libc}/${m2libcArch}/${m2libcOS}/sys/stat.c"
    "-f"
    "${m2libc}/ctype.c"
    "-f"
    "${m2libc}/stdlib.c"
  ...
  hex2_linker-0-footer_M1 = run "hex2_linker-0-footer.M1" blood-macho-0 (
    bloodFlags
    ++ [
      "-f"
      hex2_linker-0_M1
  ...
  M1-macro-1-footer_M1 = run "M1-macro-1-footer.M1" blood-macho-0 (
    bloodFlags
    ++ [
      "-f"
      M1-macro-1_M1
  ...
  hex2_linker-2-footer_M1 = run "hex2_linker-2-footer.M1" blood-macho-0 (
    bloodFlags
    ++ [
      "-f"
      hex2_linker-2_M1
  ...
  kaem-footer_M1 = run "kaem-footer.M1" blood-macho-0 (
    bloodFlags
    ++ [
      "-f"
      kaem_M1
