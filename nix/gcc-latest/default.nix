...
runCommand "gcc-${gccLatestVersion}" {
  ...
  ## gcc-latest keeps in-tree gmp/mpfr/mpc/isl: it's the compiler used
  ## to build bootstrap-gmp/mpfr/mpc/isl, so we can't reference them here
  ## without a cycle. gcc-latest-strict consumes external libs (the goal
  ## lands there). See todos task #12.
  ## Build-helpers (genmatch/gengtype/build-libcpp) compile with the chain
  ## input compiler (gcc10 + its from-stage0 libstdc++) under
  ## GCC_MODERN_HOST_BUILD_CC=0 + GCC_MODERN_WRAPPER_HOST_SHORTCUTS=0 — the
  ## pure configuration gcc-latest-strict uses.  WRAPPER_HOST_SHORTCUTS=0 also
  ...
  ## Task #13/#67: gcc-latest builds its own libgcc + libstdc++ so the
  ## resulting $out/include/c++/15.2.0 (with C++11 <type_traits>) is what
  ## gcc-latest-strict's pure-from-stage0 build-helpers compile against.
  ## Without this gcc-latest ships only the inherited gcc-4.6 headers
  ## (include/c++/4.6.4), which lack std::is_trivially_destructible, so
  ## gcc-latest-strict (HOST_BUILD_CC=0, no host clang) cannot compile
  ## gcc-15's own vec.h build-helpers.
  ## Known downstream follow-up: gcc-latest-strict may then hit
  ## cfn-operators.pd 'BUILT_IN_CBR' (a float128/genmatch mismatch) — fixed
  ## separately.
